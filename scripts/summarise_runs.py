...
def _resolve_run_dirs(inputs: list[str]) -> list[Path]:
    ...
    for item in inputs:
        ...
        if path.is_dir():
            ...
            continue
        raise ValueError(
            f"Input path is not a run directory or run-root directory: {path}"
        )
    return run_dirs
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Summarise one or more temporal verification runs."
    )
    parser.add_argument(
        "--runs",
        nargs="+",
    ...
    parser.add_argument(
        "--out", required=True, help="Output directory for tables, plots, and markdown."
    )
    parser.add_argument(
        "--manifest",
        default="",
